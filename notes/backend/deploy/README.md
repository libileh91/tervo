...
| `sprint-4.1-deploy.md` | Déploiement du frontend mobile (Sprint 4.1) |

### 4.4 Corrections d'architecture (Stage 6.1)

| Fichier | Tâche | Contenu |
|---------|:-----:|---------|
| `ports-securises.md` | INT-66 | Bind `127.0.0.1`, anatomie d'une ligne `ports:`, piège `localhost` vs `127.0.0.1`, PostgreSQL sans port |
| `healthchecks.md` | INT-67 | `depends_on` ≠ readiness, `pg_isready`, `condition: service_healthy`, démonstration réelle |
| `firewall.md` | INT-68 | ufw (22/80/443), fail2ban, 1Panel par tunnel SSH, piège « activer ufw avant SSH » |
| `ci-cd.md` | INT-70 | Double build expliqué, 2 niveaux de maturité (CI-vérifie vs registry GHCR), garde-fou `DEPLOY_ENABLED` |
...
**Un seul build**, sur le VPS. Détail complet : `ci-cd.md`.
...
| **Checks locaux *en plus* de la CI** | ✅ | Retour rapide avant push, évite de brûler des runs sur des erreurs évidentes |
