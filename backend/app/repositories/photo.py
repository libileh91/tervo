"""
ResQ — Photo Repository.

Data access layer for JobPhoto.
"""

from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from app.models.job_photo import JobPhoto


class PhotoRepository:
    """Encapsulates all database queries for photos."""

    def __init__(self, db: AsyncSession):
        self.db = db

    async def create(self, data: dict) -> JobPhoto:
        photo = JobPhoto(**data)
        self.db.add(photo)
        await self.db.commit()
        await self.db.refresh(photo)
        return photo

    async def get_by_id(self, photo_id: int) -> JobPhoto | None:
        result = await self.db.execute(select(JobPhoto).where(JobPhoto.id == photo_id))
        return result.scalar_one_or_none()

    async def delete(self, photo: JobPhoto) -> None:
        await self.db.delete(photo)
        await self.db.commit()
