...
> **Précision sur le risque RGPD :** ce n'est pas un critère de choix entre Paperless et Papra — les deux outils ont exactement le même angle mort si la photo continue de transiter par le téléphone personnel du technicien avant d'être uploadée. Ce qui règle réellement ce risque, c'est la conception de l'appli MB Chauffage elle-même : l'upload doit se faire directement depuis le job en cours dans l'appli (caméra intégrée ou upload immédiat), qui pousse ensuite la photo vers la GED via l'API — sans jamais transiter ni stationner sur la pellicule personnelle du technicien. À traiter comme une contrainte de conception de l'appli, pas comme un argument dans le match GED.
...
| **Recherche full-text**            | ✅ PostgreSQL FTS, sur du texte OCRisé donc disponible même pour les vieux scans         | 🔴 Indexation possible, mais sans OCR pro le texte des scans anciens n'existe simplement pas à indexer | **Paperless**    |
...
> **Correction v2 :** la ligne "Recherche full-text" était notée "Égal" dans la première version de ce document — c'était incohérent avec la ligne OCR juste au-dessus. Sans OCR pro, les scans anciens n'ont aucun texte extrait à indexer : la recherche de Papra sera muette sur l'essentiel du fond historique. C'est en réalité l'un des arguments les plus forts en faveur de Paperless, pas un match nul.
...
> **Angle mort de ce scénario :** il part du principe que les 5000 documents sont déjà scannés. En pratique, une partie du fond MB Chauffage est probablement encore uniquement physique (classeurs papier). Voir section 6 — le chantier de numérisation est une étape à part, en amont de l'import dans Paperless.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 3 conteneurs (web + broker + db) | À vérifier avant déploiement : espace RAM réellement disponible sur le VPS actuel, qui fait déjà tourner le Swarm de l'appli métier. ~1 Go de plus est raisonnable, mais mieux vaut prévoir un upgrade du VPS en amont plutôt qu'en urgence si la marge est faible |
...
| **AGPL-3.0**             | Contraignant seulement si des utilisateurs externes interagissent avec une version modifiée via le réseau ; pour un usage strictement interne à MB Chauffage sans modification du code, le risque réel est quasi nul — un point secondaire, pas un facteur de décision ici |
...
## 6. Chantier de numérisation des archives papier restantes

Paperless-ngx règle le problème **une fois les documents dans le système** — mais une partie du fond MB Chauffage est probablement encore uniquement physique (classeurs, factures papier non scannées). Cette bascule est un chantier à part, à planifier avant l'import massif du scénario 3 :

| Étape                     | Détail                                                                                                                                                                                        |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Inventaire**            | Estimer le volume réel de documents encore uniquement papier (par année, par type)                                                                                                            |
| **Matériel**              | Un scanner à chargeur automatique (ADF) est recommandé vu le volume — un scan à plat document par document serait ingérable sur plusieurs milliers de pages                                   |
| **Qui numérise**          | À trancher avec MB Chauffage : temps dédié d'un salarié, prestataire externe de numérisation, ou étalement progressif (ex. numériser au fil de l'eau à chaque nouvelle recherche de document) |
| **Ordre de priorité**     | Commencer par les documents les plus récents et les plus consultés (3-5 dernières années), plutôt que de viser l'exhaustivité des 20 ans dès le départ                                        |
| **Import dans Paperless** | Une fois scannés, les documents suivent le scénario 3 : dépôt dans `/consume/`, OCR automatique, classification ML après les premiers exemples classés manuellement                           |
